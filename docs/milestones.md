...
| AI patterns (structured output, tool calling, multi-step workflows) | Structured outputs: `src/lib/ai/schemas/`; realtime tool calling: `src/app/api/ai/realtime-sessions/route.ts`, `src/hooks/use-openai-realtime.ts`, `src/hooks/use-gemini-live.ts`, and `src/lib/ai/voice-proposal.ts`; multi-step recipe verification: `workflows/recipe-verification.ts` |                                                                                                                                       |
...
| Safety & security                             | input zod validation on routes, output schema validation, RLS policies, rate limiting (`src/lib/rate-limit.ts`, applied on all `api/ai/*` routes)                                                                   |
| Landing page + SEO/OG                         | `src/app/(marketing)/`, `src/app/sitemap.ts`, `src/app/robots.ts`                                                                                                                                                   |
| Pricing / credit model                        | `src/lib/credits.ts` (costs + plans), `tests/credits.test.ts` (margin assertions), `docs/credits.md` (derivation)                                                                                                   |
